...
public final class ToastUtils {
     ...
     * @param text the msg.
     */
    public static void showDuration(final CharSequence text) {
        if(sDuration!=-1){
            show(text,sDuration);
        } else {
            show(text,Toast.LENGTH_SHORT);
    ...
    public static void showAtGravity(final CharSequence text ,int gravity){
        if(sDuration!=-1){
            show(text,gravity,sDuration);
        } else {
    ...
    private static void show(final CharSequence text, final int duration) {
        //设置duration无效，设置为Toast.LENGTH_LONG 3.5秒
        HANDLER.post(new Runnable() {
            @Override
            public void run() {
                cancel();
                final Toast toast = Toast.makeText(Utils.getTopActivityOrApp(), text, Toast.LENGTH_LONG);
                sWeakToast = new WeakReference<>(toast);
                View contentView = LayoutInflater.from(Utils.getTopActivityOrApp()).inflate(R.layout.popupwindow_toast, null);
                toast.setView(contentView);
                final TextView tvMessage = toast.getView().findViewById(R.id.tv_popu);
    ...
    private static void show(final CharSequence text,final int gravity, final int duration) {
        //设置duration无效，设置为Toast.LENGTH_LONG 3.5秒
        HANDLER.post(new Runnable() {
            @Override
            public void run() {
...
//                if (sGravity != -1 || sXOffset != -1 || sYOffset != -1) {
                toast.setGravity(gravity, sXOffset, sYOffset);
//                }
                setBg(toast, tvMessage);
                tvMessage.setText(text);
                toast.show();
